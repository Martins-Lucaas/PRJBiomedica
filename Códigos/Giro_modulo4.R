...
calcular_caracteristicas_filtrado <- function(dados, tempo, nome) {
  ...
  # Aplicar filtro nível DC
  fc <- 0.1
  butterworth2 <- butter(n, c(fc/(fs/2)), type = "high")
  
  # Filtrar os dados
  dados_filtrados2 <- filtfilt(butterworth2, dados_filtrados)
  
  # Plotar sinal filtrado no domínio do tempo
  lines(tempo, dados_filtrados2, col = "blue")
  ...
  # Cálculo da FFT do sinal
  FFT_Sinal <- Mod(fft(dados_filtrados2))
  ...
  return(list(MAVSD_Filtrado = mean(abs(diff(diff(dados_filtrados2, differences = 2)))),
              F50_Filtrado = F50_Filtrado,
              Kurtosis_Filtrado = kurtosis(c(tempo, dados_filtrados2))))
}
...
TesteNormalidade_Controle_MAVSD_Filt <- shapiro.test(MAVSD_CONTROLE_Filtrado)
TesteNormalidade_ProblemaMotor_MAVSD_Filt <- shapiro.test(MAVSD_PROBLEMAMOTOR_Filtrado)

TesteNormalidade_Controle_MAVSD_Filt
TesteNormalidade_ProblemaMotor_MAVSD_Filt

# Hipótese MAVSD -------------

Teste_Tstudent_MAVSD_Filt <- wilcox.test(MAVSD_CONTROLE_Filtrado, MAVSD_PROBLEMAMOTOR_Filtrado)
Teste_Tstudent_MAVSD_Filt


# Normalidade F50 -------------

TesteNormalidade_Controle_F50_Filt <- shapiro.test(F50_CONTROLE_Filtrado)
TesteNormalidade_ProblemaMotor_F50_Filt <- shapiro.test(F50_PROBLEMAMOTOR_Filtrado) 

TesteNormalidade_Controle_F50_Filt
TesteNormalidade_ProblemaMotor_F50_Filt

# Hipótese F50 ------------

Teste_Hipótese_F50_Filt <- wilcox.test(F50_CONTROLE_Filtrado, F50_PROBLEMAMOTOR_Filtrado)
Teste_Hipótese_F50_Filt

# Normalidade Kurtosis ------------

TesteNormalidade_Controle_Kurtosis_Filt <- shapiro.test(KURTOSIS_CONTROLE_Filtrado)
TesteNormalidade_ProblemaMotor_Kurtosis_Filt <- shapiro.test(KURTOSIS_PROBLEMAMOTOR_Filtrado)

TesteNormalidade_Controle_Kurtosis_Filt
TesteNormalidade_ProblemaMotor_Kurtosis_Filt

# Hipótese Kurtosis -----------

Teste_Hipótese_Kurtosis_Filt <- wilcox.test(KURTOSIS_CONTROLE_Filtrado, KURTOSIS_PROBLEMAMOTOR_Filtrado)
Teste_Hipótese_Kurtosis_Filt